...
Original file is located at
    https://colab.research.google.com/drive/1vuyntqOVCJqm3qtONil1YktROf1aHbwQ
...
topFiveErrURLs = endpointSum.takeOrdered(5, lambda pair: (-1)*pair[1])
...
hourGroupedHosts = hourToHostPairTuple.distinct().groupByKey().sortByKey()

hourHostCount = hourGroupedHosts.map(lambda pair: (pair[0], len(list(pair[1]))))
...
hoursWithHosts = hourlyHosts.map(lambda pair: pair[0]).collect()
...
avgHourlyReqPerHost = (sortedByHour.join(hourlyHosts).mapValues(lambda pair: pair[0] // pair[1]).sortByKey()).cache()
...
badEndpointsTop10 = badEndpointsSum.top(10, key = (lambda pair: pair[1]))
